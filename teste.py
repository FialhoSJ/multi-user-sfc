...
# --- Bloco de Teste de Unidade (Executa apenas se rodar o arquivo diretamente) ---
if __name__ == "__main__":
    # Cenário de Teste
    DURACAO = 1000.0
    N_FALHAS = 3
    MIN_DUR = 20.0
    CONF = 0.95  # Alvo: ~50s de downtime total

    try:
        resultado = calcular_janelas_falha(DURACAO, N_FALHAS, MIN_DUR, CONF)
        
        total_downtime_real = sum(d for _, d in resultado)
        
        print(f"--- Relatório de Geração ---")
        print(f"Target Downtime: {DURACAO * (1 - CONF):.2f}s")
        print(f"Real Downtime:   {total_downtime_real:.2f}s")
        print(f"Diferença:       {abs((DURACAO * (1 - CONF)) - total_downtime_real):.4f}s")
        print("\nCronograma Gerado:")
        for i, (inicio, duracao) in enumerate(resultado, 1):
            print(f"  Falha #{i}: Inicia em {inicio:>6.2f}s | Dura {duracao:>5.2f}s")
