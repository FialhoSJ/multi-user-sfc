...
from algorithms.networkUtils import get_link_bandwidth_free, get_link_latency, calculate_latency_betwen_nodes
...
class Musfico():
        ...
        for node in self.graph.nodes():
            ...
            for i in range(0, length - 1):
                ...
                if edge_key in bandwidth_usage_info:
                    ...
                    residual_bandwidth = get_link_bandwidth_free(self.graph, u, v) - bandwidth_request
                ...
                if previous_vnf_id not in ['src', 'dst']:
                    real_comm_latency += calculate_latency_betwen_nodes(self.graph, u, v, previous_vnf)
    ...
    def _dp(self, substrate_node, vnf):
        ...
        for node in self.graph.nodes():
            # Guard Clauses
            if node == substrate_node: continue
            if node in self.node_info[substrate_node][previous_vnf_id]['current_substrate_nodes']: continue
            if node == self.src_substrate_node or node == self.dst_substrate_node: continue
            ...
            if not self.node_info[node][vnf_id]['latency'] or total_latency <= self.node_info[node][vnf_id]['latency']:
                ...
                self.node_info[node][vnf_id]['src_path'] = self.node_info[substrate_node][previous_vnf_id]['src_path'][:] + path[:-1]
