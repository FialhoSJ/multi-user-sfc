...
class Kuririn:
    def __init__(self, model_name):
        ...
        self.model_path = f'saved_models_rl/{model_name}_sfc_allocation'
        self.name = "Kuririn"
        self.env = None
        self.model = None
    ...
    def install_substrate_network(self, substrate_network, sfc_list, shareable_sfs=[]):
        ...
        self.num_nodes = len(substrate_network)
    ...
    def start_algorithm(self, shareable_sfs={}, is_backup=False):
        self.is_backup = False
        if self.algorithm(self.substrate_network, self.sfc, shareable_sfs):
            # self._save_model()
            return True
        # self._save_model()
        return False
    ...
    def algorithm(self, substrate_network, sfc, shareable_sfs):
        ...
        route_info, latency = self.find_best_allocation_for_sfc(
            G, substrate_network,service_requirements, nodes_resource,services, dst
        )
    ...
    def create_network_graph(self, network_topology):
        ...
        for node, edges in network_topology.items():
            for target, attr in edges.items():
                G.add_edge(node, target, bandwidth=attr['bandwidth_free'], weight=1)
        return G
    ...
    def set_nodes_resources(self, substrate_network, shareable_sfs={}):
        ...
        for server in net_info.nodes:
            resources[server] = {
                'cpu_capacity': round(net_info.get_node_cpu_capacity(server), 3),
                'cache_capacity': round(net_info.get_node_cache_capacity(server), 3),
                'cpu_used': round(net_info.get_node_cpu_used(server), 3),
                'cache_used': round(net_info.get_node_cache_used(server), 3),
                'cpu_free': round(net_info.get_node_cpu_free(server), 3),
                'cache_free': round(net_info.get_node_cache_free(server), 3),
    ...
    def find_best_allocation_for_sfc(self, G, substrate_network,service_requirements, server_resources, services, dst):    
        if not self.env:
            self.env = NetworkEnv(G, substrate_network,server_resources, services ,service_requirements, 
