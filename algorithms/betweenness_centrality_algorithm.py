        ...
        def helper(substrate_network, sfc, vnf_list, head, tail, src, dst):
            ...
            if head < tail:
                ...
                if vnf_list[middle_index].id not in map_res:
                    ...
                    sorted_bc = sorted(list(bc.items()), key=lambda kv: (kv[1], kv[0]), reverse=True)
                    # print sorted_bc
                    ...
                    while True:
                        try:
                            ...
                            if candidate_node not in used_nodes and (cpu_available >= cpu_request or cache_available >= cache_request):
                                break
                            count += 1
                        except:
