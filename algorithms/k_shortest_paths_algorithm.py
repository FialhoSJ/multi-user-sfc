...
#                if residual_cpu_capacity > max_residual_cpu_capacity or residual_cache_capacity > max_residual_cache_capacity:
            ...
            head_cache = -1 if candidate_node_head == None else substrate_network.get_node_cache_free(candidate_node_head)

            tail_cpu = -1 if candidate_node_tail == None else substrate_network.get_node_cpu_free(candidate_node_tail)
            tail_cache = -1 if candidate_node_tail == None else substrate_network.get_node_cache_free(candidate_node_tail)

            candidate_node_n = candidate_node_head if head_cpu > tail_cpu else candidate_node_tail
            candidate_node_n_cache = candidate_node_head if head_cache > tail_cache else candidate_node_tail

            if candidate_node_n == None or candidate_node_n_cache == None:
        ...
        for node in longest_path:
            ...
            if node not in used_node:
                ...
                if cpu_request > cpu_available:
                    ...
                    if substrate_network.get_node_cpu_used(e[1]) + cpu_request > substrate_network.get_node_cpu_capacity(e[1])*cpu_overload_rate:
                        logger.debug("node %s has not sufficient overloaded cpu usage", e[1])
                        continue
                   
                    extra_proc_latency = cpu_request / (substrate_network.get_node_cpu_capacity(e[1])*cpu_overload_rate - substrate_network.get_node_cpu_used(e[1]) )
                    #continue
                if cache_request > cache_available:
                     # if node has not sufficient cpu, check next edge.
                    logger.warn("node %s has not sufficient cache. Available: %s", node, cache_available)
                    if substrate_network.get_node_cache_capacity(e[1]) == 0:
                        continue

                    # if node has sufficient cpu for over loaded usage.
                    if substrate_network.get_node_cache_used(e[1]) + cache_request > substrate_network.get_node_cache_capacity(e[1])*cache_overload_rate:
                        logger.debug("node %s has not sufficient overloaded cache usage", e[1])
                        continue
                    extra_proc_latency_cache = cache_request / (substrate_network.get_node_cache_capacity(e[1])*cache_overload_rate - substrate_network.get_node_cache_used(e[1]) )
                    #continue
        ...
        if len(route_info) < number_of_vnfs + 1:
            ...
            while current_vnf.id != dst_vnf.id:
                ...
                for i in range(0, length - 1):
                    ...
                    latency = latency + substrate_network.get_link_latency(path[i], path[i + 1]) + extra_proc_latency
