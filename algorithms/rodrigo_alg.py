...
class Rodrigo(Algorithm):
    ...
    def algorithm(self,substrate_network, sfc, shareable_sfs=None):
        ...
        route_info, latency = self.fit_path(G,service_requirements,server_resources,services,dst)
    ...
    def fit_path(self, G, service_requirements, server_resources, services, dst):
        ...
        for service in services_list:
            ...
            best_server, best_path, total_cost = self.find_best_server_for_service(G, server_resources_local,
                                                                                    service_requirements_local,
                                                                                    current_location, service,
    ...
    def find_best_server_for_service(self,G,server_resources,service_requirements,current_location, service, bandwidth_requirement):
        ...
        for server, num_hops in paths.items():
            ...
            if all(G[u][v]['bandwidth'] >= bandwidth_requirement for u, v in zip(path, path[1:])):
                ...
                cache_required = 0 if reuse else service_requirements[service]['cache']  # Assuming there's a cache requirement

                # Calculate available resources
                available_cpu = server_resources[server]['cpu_free']      #- server_resources[server]['cpu_used']
                available_cache = server_resources[server]['cache_free']  #- server_resources[server]['cache_used']

                if server_resources[server]['cpu_used'] >= 17.27 or reuse: # já estava ligado
                    boot_cost == 0 
                elif (server_resources[server]['cpu_used'] + cpu_required) >= 17.27: # tem que ligar
                    boot_cost = 1
                else:
                    boot_cost = 0
        ...
        if best_path and best_server != current_location:  # Evita atualizar banda se alocação é no mesmo servidor
