...
class NewAlg(Algorithm):
    ...
    def _compute_cost(self, length=0, bw_saved=0, cpu_saved=0, cache_saved=0, latency=0):
        ...
        #
        #self.weights
        normalized_bw = bw_saved / (500 * length)
        normalized_cpu = cpu_saved / 100
    ...
    def algorithm(self,substrate_network, sfc):
        ...
        for node, latency in list(node_latency.items()):
            ...
            cost = self._compute_cost(length = length, bw_saved = total_band_saved, cpu_saved = total_cpu_saved, 
                                      cache_saved = total_cache_saved, latency = total_latency)
            if not self.node_info[dst_substrate_node][dst_vnf.id]['cost'] \
    ...
    def _dp(self, substrate_node, vnf):
        ...
        for node, latency in list(node_latency.items()):
            ...
            cost = self._compute_cost(length = length, bw_saved = total_band_saved, cpu_saved = total_cpu_saved, 
                                      cache_saved = total_cache_saved, latency = total_latency)
            if not self.node_info[node][vnf_id]['cost'] or (_cost + cost) <= self.node_info[node][vnf_id]['cost']:
