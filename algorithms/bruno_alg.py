...
class BrunoAlg(Algorithm):
    ...
    def algorithm(self,substrate_network, sfc):
        ...
        shortest_paths = k_shortest_paths(self.substrate_network, source=previous_vnf_node, 
                                                     target=dst_substrate_node, weight='latency', k=5)
        for path in shortest_paths:
            is_enough_bandwidth = True
            path_latency = 0
            for i in range(len(path) - 1):
                required_bandwidth = self.substrate_network.get_link_bandwidth_free(path[i],
    ...
    def _alg(self, vnf):
        ...
        previous_vnf_node = self.src_substrate_node if previous_vnf.id == 'src' else self.path_info[previous_vnf.id]['substrate_node']
        ...
        for node, sfs in self.shareable_sfs.items():
            if vnf.id in list(map(lambda sf: sf.id, sfs)):
                ...
                for i in range(len(candidate_path) - 1):
                    required_bandwidth = self.substrate_network.get_link_bandwidth_free(candidate_path[i],
                                                                    candidate_path[i + 1]) - bandwidth_request
                    if required_bandwidth < 0:
                        is_enough_bandwidth = False
                        break
                if not is_enough_bandwidth:
