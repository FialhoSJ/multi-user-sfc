...
from muar_sfc.utils.network_utils import calcular_percentual_cpu_total, calcular_percentual_cache_total, calculate_total_latency
...
class SFC_AllocationEnv_hephaestus(gymnasium.Env):
    ...
    def __init__(self,
        ...
        self.observation_space = spaces.Dict({
            ...
            "app_metrics": spaces.Box(low=0, high=1, shape=(num_app_features,), dtype=np.float32),

            # Métricas da infraestrutura (análogo ao "Cluster" do artigo)
            "cluster_metrics": spaces.Box(low=0, high=1, shape=(num_nodes, num_cluster_features), dtype=np.float32),
        })
    ...
    def _get_obs(self) -> Dict[str, np.ndarray]:
        ...
        num_valid_nodes = len(self.valid_nodes)
        cluster_metrics = np.zeros(self.observation_space["cluster_metrics"].shape, dtype=np.float32)

        # Para normalizar a capacidade, encontramos o máximo na rede
        max_total_cpu = max(data['cpu_capacity'] for _, data in self.graph.nodes(data=True) if data.get('cpu_capacity'))
        max_total_cache = max(data['cache_capacity'] for _, data in self.graph.nodes(data=True) if data.get('cache_capacity'))
        if max_total_cpu == 0: max_total_cpu = 1
        if max_total_cache == 0: max_total_cache = 1
