...
class KShortestPathsAlgorithm(Algorithm):
    ...
    def algorithm(self, substrate_network, sfc):
        ...
        k_shortest_paths_res = k_shortest_paths(
            substrate_network, src_substrate_node, dst_substrate_node, k, "latency"
        )
        ...
        while True:  # length contains src and dst, thus minus 2
            ...
            if candidate_node_head is not None:
            ...
            if candidate_node_tail is not None:
                used_node.append(candidate_node_tail)

            # compare two nodes and find the one with max residual
            candidate_node_n = None
            candidate_node_n_cache = None

            head_cpu = (
                -1
                if candidate_node_head is None
                else substrate_network.get_node_cpu_free(candidate_node_head)
            )
            head_cache = (
                -1
                if candidate_node_head is None
                else substrate_network.get_node_cache_free(candidate_node_head)
            )

            tail_cpu = (
                -1
                if candidate_node_tail is None
                else substrate_network.get_node_cpu_free(candidate_node_tail)
            )
            tail_cache = (
                -1
                if candidate_node_tail is None
                else substrate_network.get_node_cache_free(candidate_node_tail)
            )

            candidate_node_n = candidate_node_head if head_cpu > tail_cpu else candidate_node_tail
            candidate_node_n_cache = (
                candidate_node_head if head_cache > tail_cache else candidate_node_tail
            )

            if candidate_node_n is None or candidate_node_n_cache is None:
                logger.warn("candidate node is None")
                return False

            shortest_path_m_head_n = substrate_network.get_minimum_latency_path(
                m_head, candidate_node_n
            )
            shortest_path_n_m_tail = substrate_network.get_minimum_latency_path(
                candidate_node_n, m_tail
            )
            if shortest_path_n_m_tail is None or shortest_path_m_head_n is None:
                logger.warn("have not shortest path")
                return False

            longest_path = (
                longest_path[:index]
                + shortest_path_m_head_n
                + shortest_path_n_m_tail[1:-1]
                + longest_path[index + 1 :]
            )
        ...
        extra_proc_latency = 0
        cpu_overload_rate = 3
        cache_overload_rate = 3
        ...
        for node in longest_path:
            ...
            if node not in used_node:
                ...
                cache_request = sfc.get_vnf_cache_request(sfc.get_next_vnf(current_vnf))

                cpu_available = substrate_network.get_node_cpu_free(node)
                cache_available = substrate_network.get_node_cache_free(node)
                if cpu_request > cpu_available:
                    # if node has not sufficient cpu, check next edge.
                    logger.warn(
                        "node %s has not sufficient cpu. Available: %s", node, cpu_available
                    )
                    if substrate_network.get_node_cpu_capacity(e[1]) == 0:
                        continue

                    # if node has sufficient cpu for over loaded usage.
                    if (
                        substrate_network.get_node_cpu_used(e[1]) + cpu_request
                        > substrate_network.get_node_cpu_capacity(e[1]) * cpu_overload_rate
                    ):
                        logger.debug("node %s has not sufficient overloaded cpu usage", e[1])
                        continue

                    extra_proc_latency = cpu_request / (
                        substrate_network.get_node_cpu_capacity(e[1]) * cpu_overload_rate
                        - substrate_network.get_node_cpu_used(e[1])
                    )
                    # continue
                if cache_request > cache_available:
                    # if node has not sufficient cpu, check next edge.
                    logger.warn(
                        "node %s has not sufficient cache. Available: %s", node, cache_available
                    )
                    if substrate_network.get_node_cache_capacity(e[1]) == 0:
                        continue

                    # if node has sufficient cpu for over loaded usage.
                    if (
                        substrate_network.get_node_cache_used(e[1]) + cache_request
        ...
        if len(route_info) < number_of_vnfs + 1:
            ...
            while current_vnf.id != dst_vnf.id:
                ...
                for i in range(0, length - 1):
                    ...
                    latency = (
                        latency
                        + substrate_network.get_link_latency(path[i], path[i + 1])
                        + extra_proc_latency
                    )
                    logger.debug("extra processing latency: %s", extra_proc_latency)
