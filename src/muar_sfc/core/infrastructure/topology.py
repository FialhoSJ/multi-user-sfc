...
class NetworkTopology:
    ...
    def add_wired_link(
        self, node1: Any, node2: Any, bandwidth: float = 1000.0, latency: float = 1.0
    ) -> None:
        ...
        if not self._infra_graph.has_node(node1) or not self._infra_graph.has_node(node2):
            raise ValueError(
                f"Nós {node1} ou {node2} não existem na infraestrutura para criar link."
            )
    ...
    def add_wireless_link(
        self, mobile_id: Any, router_id: Any, bandwidth: float = 100.0, latency_base: float = 5.0
    ) -> None:
