...
class BackupManager:
    ...
    def __init__(self, args):
        ...
        if user_wants_backup and not is_target_alg:
            print(
                f"[BackupManager] INFO: Backup proativo desativado. '{args.alg}' não suporta essa estratégia."
            )
    ...
    def escolher_src_dst(
        self, dicionario: Dict, vnf_escolhida: str, latency_limit: int = 10
    ...
    def seletive_strategy(
        ...
        for sfc_id in active_sfcs:
            ...
            for vnf_info in sfc.vnfs_dict:
                ...
                dst_node, src_node, latency_req = self.escolher_src_dst(
                    sfc_rf, vnf_id, original_latency
                )
                ...
                backup_sf_list = [
                    ...
                    {
                        "type": 2,
                        "name": backup_vnf_name,
                        "CPU": cpu_req,
                        "cache": cache_req,
                        "in_bw": 0,
                        "out_bw": vnf_info["out_bw"] * reduction_factor,
                        "latency": 0,
    ...
    def create_contextual_mini_sfc(
        ...
        target_vnf_info = next(
            (info for info in original_sfc.vnfs_dict if info["name"] == vnf_to_replicate_id), None
        )
