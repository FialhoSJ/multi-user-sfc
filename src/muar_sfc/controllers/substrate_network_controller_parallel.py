...
class SubstrateNetworkControllerP:
    ...
    def check_sfc_duration(self) -> None:
            ...
            while not self.is_stopped:
                with self.lock:
                    ...
                    if elapsed_time < self.update_interval:
                        ...
                        # Se o tempo de execução for maior que o intervalo, reexecuta imediatamente
                        sleep_time = 0
        ...
        thread.daemon = (
            True  # Faz a thread rodar como daemon para que o programa termine sem bloqueios
        )
        thread.start()
            ...
            while not self.is_stopped:
                with self.lock:
                    ...
                    if len(self.players_sfc_list) != 0:
                        sfcs_moved, new_locations = (
                            self.mobility_manager.check_all_vehicles_position_changes()
                        )
                    ...
                    # # Abre o arquivo de log em modo append para não sobrescrever
    ...
    def recover_sfcs(self, sfc_affected):
        ...
        #     # TODO sfcs que foram afetadas pelo crasher não se
        #     # movimentam mais por comodidade de código
        #     self.mobility_manager.remove_sfc(sfc_id)
        #     sfc = self.substrate_network.get_sfc_by_id(sfc_id)
        ...
        def task_backup():
            while not self.is_stopped:
                with self.lock:
                    ...
                    filtered_nodes = {
                        node: rel for node, rel in nodes_rel.items() if rel > threshold
                    }
                    top_1_node = dict(
                        sorted(filtered_nodes.items(), key=lambda item: item[1], reverse=True)[:2]
                    )
    ...
    def send_back_to_qeue(
        self, sfc_list, changed_location=False, new_location=False, punishment=10
    ):
        # A duração deve ser a mesma para as duas
        sfc_id_duration = self.sfc_manager.sfc_id_duration
        first_sfc = sfc_list[0]
        duration = sfc_id_duration[first_sfc.id]["duration"] - (
            time.time() - sfc_id_duration[first_sfc.id]["timer"]
        )
        new_sfc_list = []
        for sfc in sfc_list:
            sfc_id = sfc.id
            location = new_location if new_location else sfc.dst.substrate_node
            new_vnfs_list_dict = copy.deepcopy(sfc.vnfs_dict)

            # Checar depois se esse procedimento não é necessário também para Unique
            if changed_location and re.search("cache", sfc_id) is not None:
                ...
                if ma_old_key in old_routing_info[sfc_id]:
                ...
                if re_old_key in old_routing_info[sfc.id]:
            ...
            new_sfc = SFCGenerator(
                new_sfc_dict
            ).generate()  # gera uma nova e coloca de volta na fila
    ...
    def output_results(self, results_dict, sfc_id, res_output=False) -> None:
        def output_network_resources(deploy_time):
            ...
            self.output_writter.output_nodes_sf_utilization(
                self.substrate_network, deploy_time
            )
        ...
        """Updates the network state and check for resource overhead."""
        if results_dict and not res_output:
            # output of the simulation
            output_network_resources(deploy_time=results_dict["current_time"])
            output_flows(
                results_dict["current_time"],
                sfc_id,
                results_dict["latency"],
                results_dict["run_duration"],
                results_dict["is_success"],
                results_dict["backup_sfc"],
                results_dict["fail_reason"],
            )
        ...
        if sfc_id in sfcs_crash_aff:
            if not self.sfcs_crash_affected[sfc_id]["backup_success"]:  #
                time_to_recover = None
                latency_diff = None
                latency_factor = None
                resource_factor = None
                if results_dict:
                    self.sfcs_crash_affected[sfc_id]["recover_success"] = results_dict[
                        "is_success"
                    ]

                    if results_dict["is_success"]:
                        time_to_recover = (
                            time.time() - self.sfcs_crash_affected[sfc_id]["fall_time"]
                        )  # if results_dict['is_success'] else None
                        latency_diff = (
                            results_dict["latency"]
                            - self.sfcs_crash_affected[sfc_id]["old_latency"]
                        )  # if results_dict['is_success'] else None
                        latency_factor = (
                            results_dict["latency"]
                            - self.sfcs_crash_affected[sfc_id]["old_latency"]
                        ...
                        old_resource_reuse = self.sfcs_crash_affected[sfc_id][
                            "resource_info"
                        ]  # Taxa antiga

                        if old_resource_reuse != 0 and old_resource_reuse != 0.0:
                            resource_factor = (
                                old_resource_reuse - results_dict["resource_info"]
                            ) / old_resource_reuse
                        else:
                            resource_factor = 0
        ...
        if self.verbose:
            ...
            self.output_writter.print_output_info(
                self.substrate_network, self.success
    ...
    #         top_1_node = dict(
    #             sorted(filtered_nodes.items(), key=lambda item: item[1], reverse=True)[:1]
    #         )
    ...
    def deploy_sfc(self, sfc: object) -> bool:
        ...
        if results_dict:
            ...
            return results_dict
        return False
        # # Se não é Backup e tiver sido sucesso, adiciona um veículo para essa SFC;
    ...
    def sequential_submit_sfcs(self):
        ...
        while self.sfc_queue.qsize() != 0:
            player_sfc_id_list = []
            deploy_log = {}  # Indicador para verificar se todas as SFCs foram um sucesso
            all_success = True
            ...
            if not result_dict["backup_sfc"]:
                if all_success:
                    ...
                    time.time()
                    # for sfc in sfc_list:
                    #     self.sfc_manager.sfc_id_duration[sfc.id] = {
                    #         "duration":sfc.duration,
                    #         "timer":sfc_duration_timer
                    #     }
    ...
    def sequential_crasher(self, interval=25):
        ...
        vnfs_backup_util = 0
        if self.alg.name == "ga":
            for _backup_id, original_sfc in backups.items():
                sfc_backups = self.sfc_manager.backup_manager.sfcs_backups_instatiated[
                    original_sfc
                ]
        ...
        for server in nodes_to_crash:
            ...
            # Remove as sfcs de backup da análise e monta uma estrutura boa.
            for sfc_vnf in server_info:
        ...
        for sfc_id, vnfs in sfcs_crashed_ids.items():
            ...
            if backup_found:
                ...
                if self.alg.name == "ga":
                    ...
                    keys = list(new_rf)
                    if vnf_id in keys[:-1]:
                        new_rf[keys[keys.index(vnf_id) + 1]] = backup_found["route_info"][
                            "source"
                        ]
                    ...
                    self.sfc_manager.undeploy_sfc(
                        sfc_id, self.substrate_network, take_out_backup=False
                    )  # não retira o backup, pois ele foi ativado
                ...
                self.sfcs_crash_affected[sfc_id] = {
                    ...
                    "backup_success": True,
                    "latency_diff": l_diff,
                    "old_latency": o_latency,
                    "latency_degrad": latency_degrad,
                    "resource_degrad": resource_degrad,
                    "time_to_recover": time_to_r,
                    "backup_efficient": backup_efficient,
                }
        ...
        for sfc_id in list(sfcs_crashed_ids.keys()):
            ...
            self.sfc_manager.calculate_latency(o_rf)
    ...
    def check_timer_qeue(self):
        ...
        #     # Nova lista para armazenar itens que não atendem à condição
        #     new_timer_qeue_sfcs = []
        ...
        #         else:
        #             new_timer_qeue_sfcs.append(entry)  # Mantém na lista se não atender
    ...
    def sequential_mobility(self, interval=5):
        ...
        if len(self.players_sfc_list) != 0:  # Se não houver mais SFC's não faz nada
            ...
            for sfc_list, new_location in zip(sfcs_moved, new_locations, strict=False):
                ...
                # try:
                obj_sfc_list = [
                    self.substrate_network.get_sfc_by_id(sfc_id) for sfc_id in sfc_list
                ]
                self.send_back_to_qeue(
    ...
    def sequential_operation(self):
        ...
        while not self.is_stopped:
            ...
            if self.fail_manager.activated:
                ...
                if (
                    (current_time - last_crasher_time >= crasher_interval)
                    and not self.backup_in_qeue()
                    and self.crashs_trials < crash_limit
                ):
                    # self.sequential_backup()
                    self.crashs_trials = self.crashs_trials + 1
                    self.sequential_crasher()
                    # if retorno != False:
                    #     self.sequential_submit_sfcs()
