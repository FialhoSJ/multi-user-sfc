...
class LuxembourgV2(TopologyBase):
    ...
    def get_topology_info(self):
        ...
        edges = self.topology
        edge_computing_servers = self.edge_computing_servers
        return {
            "nodes": nodes,
            "edges": edges,
...
if __name__ == "__main__":
    ...
    # posições para o layout (normaliza os valores para um desenho melhor)
    pos = {node: (x/1000, y/1000) for node, (x, y) in lux.positions.items()}

    # diferencia nós servidores e roteadores por cor
    node_colors = []
    for node in G.nodes():
        if node == 0:
