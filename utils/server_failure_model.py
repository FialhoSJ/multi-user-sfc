...
class ServerFailureModel:
    ...
    def __init__(self, eta: float, beta: float, alpha: float, lambda_base: float):
        ...
        """
        if beta <= 1.0:
            print(f"Warning: beta = {beta} <= 1. This models a constant or decreasing failure rate, not wear-out.")
    ...
    def software_aging_rate(self, t_uptime: float) -> float:
        ...
        """
        if t_uptime < 0: return self.lambda_base
        return self.lambda_base + self.alpha * t_uptime

    def check_failure(self, global_time: float, uptime_time: float, step_duration: float, cpu_stress_factor: float = 1.0) -> bool:
        """
...
if __name__ == '__main__':
    ...
    for t in range(total_steps):
        ...
        if server_model.check_failure(global_time, uptime_time, step_duration, cpu_stress_factor=cpu_stress):
            print(f"Server FAILED at global_time={global_time}h, uptime={uptime_time}h (CPU Stress: {cpu_stress:.2f})")
